...
#if defined(BCMDBG) || defined(DHD_DEBUG) || defined(WLTEST)
extern char acname[4][3];
#endif
...
#define BCMSTRBUF_ORIGBUF(b)	((b)->origbuf)
...
extern uint8 dscp2up(uint8 *up_table, uint8 dscp);
...
#endif	/* BCMDRIVER */
/* ** end of driver-only section ** */

#define US_PER_SECOND		1000000 /* 1 sec = 1000000 us */
#define TSF_TICKS_PER_MS	1000

#define TAF_UNITS_TO_MICROSEC(a)	(((a) + 8) >> 4)
#define TAF_MICROSEC_TO_UNITS(a)	((a) << 4)
...
bool bcm_has_option(const char *option, int argc, char *argv[], uint *value);
bool bcm_has_option_str(const char *option, int argc, char *argv[], const char **value);
bool bcm_has_option_ex(const char *option, char *args, uint *value);
int bcm_tokenize_zsv(char *buffer, uint length, char **argv, uint *argc, uint num);
...
#if defined(BCMDBG) || defined(DHD_DEBUG) || defined(WLTEST)
extern char *stalltype2str(char stall_type);
#endif
extern char *stallmodule2str(char stall_module);
...
/* Test whether in range [min, max] */
#ifndef TEST_IN_RANGE
#define TEST_IN_RANGE(x, min, max) \
	(((x) >= (min)) && ((x) <= (max)))
#endif /* TEST_IN_RANGE */
...
extern void clr_bitrange_u32(void *array, uint start_pos, uint end_pos, uint maxbit_pos);
...
/* Given a bitmap position [0..maxbits), get the index into uint32 array */
...
extern int bcm_bputc(struct bcmstrbuf *b, int c);
extern int bcm_bprintf(struct bcmstrbuf *b, const char *fmt, ...);
extern int bcm_bprintf_val_pcent(bcmstrbuf_t *b, uint32 val, uint32 pcent, int pad);

/**
 * Helper macro that will print to kernel log if passed in buffer is NULL.
 */
#define bcm_bprintf_ex(buf, fmt, ...) \
	((buf) ? bcm_bprintf(buf, fmt, ##__VA_ARGS__) : printf(fmt, ##__VA_ARGS__))
...
typedef struct bcm_mwbmap bcm_mwbmap_t;

#if defined(DONGLEBUILD)
#define BCM_MWBMAP_USE_CNTSETBITS       /* runtime count set bits */
#ifndef BCM_MWBMAP_ITEMS_MAX
#define BCM_MWBMAP_ITEMS_MAX    (16 * 1024)
#endif
#else  /* ! DONGLEBUILD */
#define BCM_MWBMAP_ITEMS_MAX    (64 * 1024)  /* May increase to 64K */
#endif /* DONGLEBUILD */

#define BCM_MWBMAP_INVALID_HDL	((bcm_mwbmap_t *)NULL)
#define BCM_MWBMAP_INVALID_IDX	((uint32)(~0U))

/* Incarnate a multiword bitmap based small index allocator */
extern bcm_mwbmap_t * bcm_mwbmap_init(osl_t * osh, uint32 items_max);

/* Free up the multiword bitmap index allocator */
extern void bcm_mwbmap_fini(osl_t * osh, bcm_mwbmap_t * mwbmap_hdl);

/* Allocate a unique small index using a multiword bitmap index allocator */
extern uint32 bcm_mwbmap_alloc(bcm_mwbmap_t * mwbmap_hdl);

/* Force an index at a specified position to be in use */
extern void bcm_mwbmap_force(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix);

/* Free a previously allocated index back into the multiword bitmap allocator */
extern void bcm_mwbmap_free(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix);

/* Fetch the toal number of free indices in the multiword bitmap allocator */
extern uint32 bcm_mwbmap_free_cnt(bcm_mwbmap_t * mwbmap_hdl);

/* Determine whether an index is inuse or free */
extern bool bcm_mwbmap_isfree(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix);

/* Debug dump a multiword bitmap allocator */
extern void bcm_mwbmap_show(bcm_mwbmap_t * mwbmap_hdl);

extern void bcm_mwbmap_audit(bcm_mwbmap_t * mwbmap_hdl);
...
#define MASK_32_BITS		((uint32)(~0))
#define MASK_8_BITS		((1 << 8) - 1)
#define BITMASK(POS, NBITS)	(((1 << NBITS) - 1) << POS)
...
#define MAP_PKTS_CB_NOOP    (0)
#define MAP_PKTS_CB_DIRTY   (1 << 1)
typedef uint32 (*map_pkts_cb_fn)(void *ctx, void *pkt);
...
#ifdef SHOW_LOGTRACE
#ifdef EVENT_LOG_NIC
#define TRACE_LOG_BUF_MAX_SIZE	(3072u)
#else
#define TRACE_LOG_BUF_MAX_SIZE	(1900u)
#endif /* EVENT_LOG_NIC */
...
/*
 * Macro to be instantiated when debugging. Example invocations:
 * PRINT_LOC("")		->	'LOC:foo@131         # meaning: function foo, line 131
 * PRINT_LOC("bar")		->	'LOC:foo@131 bar'
 * PRINT_LOC("bar=%d", bar)	->	'LOC:foo@131 bar=3'
 * PRINT_LOC("%d,%d", bar, bar)	->	'LOC:foo@131 3,3'
 */
#define PRINT_LOC(fmt, ...) printf("LOC:%s@%d " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
...
#define BCM_PRINT			printf
#define BCM_BPRINT(b, fmt, ...) ((b) ? \
	bcm_bprintf(b, fmt, ##__VA_ARGS__) : \
	BCM_PRINT(fmt, ##__VA_ARGS__))

extern uint32 bcm_running_avg(uint32 cur_val, uint32 new_val, uint16 sample_size);
#define BCM_RUN_AVG(cur_val, new_val, sample_size) \
	bcm_running_avg(cur_val, new_val, sample_size)

#if defined(BCM_ROUTER) && defined(BCMDRIVER) && !defined(DONGLEBUILD)
typedef void (*bcm_panic_cb_t)(void *drvr);
extern void bcm_radio_register(uint8 unit, void *drvr, bcm_panic_cb_t panic_cb);
extern void bcm_radio_unregister(uint8 unit, void *drvr);
extern void bcm_radio_panic_hook(void);

#if !defined(CMWIFI)
typedef struct bcm_soc_info {
	uint32 process;
	uint32 substrate;
	uint32 foundry;
} bcm_soc_info_t;
extern int bcm_get_soc_info(bcm_soc_info_t *soc_info);
#endif /* !CMWIFI */
#endif /* BCM_ROUTER && BCMDRIVER && !DONGLEBUILD */
