...
bcm_tlv_t *bcm_find_tlv(const  void *buf, int buflen, uint key);
#define bcm_parse_tlvs(buf, buflen, key) bcm_find_tlv((buf), (buflen), (key)) /* obsolete fn name */

/* find the tlv for a given ext id */
bcm_tlv_t *bcm_find_tlv_ext(const  void *buf, int buflen, uint key, int key_ext);
#define bcm_parse_tlvs_ext(buf, buflen, key, key_ext) \
	bcm_find_tlv_ext((buf), (buflen), (key), (key_ext)) /* obsolete fn name */

/*
 * Traverse tlvs and return pointer to the first tlv that
 * matches the key. Return NULL if not found or tlv len < min_bodylen
 */
bcm_tlv_t *bcm_find_tlv_min_bodylen(const  void *buf, int buflen, uint key, int min_bodylen);
#define bcm_parse_tlvs_min_bodylen(buf, buflen, key, min_bodylen) \
	bcm_find_tlv_min_bodylen((buf), (buflen), (key), (min_bodylen)) /* obsolete fn name */

/* parse tlvs for dot11 - same as parse_tlvs but supports 802.11 id extension */
bcm_tlv_t *bcm_find_tlv_dot11(const  void *buf, int buflen, uint key, bool id_ext);
#define bcm_parse_tlvs_dot11(buf, buflen, key, id_ext) \
	bcm_find_tlv_dot11((buf), (buflen), (key), (id_ext)) /* obsolete fn name */

/* same as parse_tlvs, but stops when found id > key */
const  bcm_tlv_t *bcm_find_tlv_ordered(const  void *buf, int buflen, uint key);
#define bcm_parse_ordered_tlvs(buf, buflen, key) \
	bcm_find_tlv_ordered((buf), (buflen), (key)) /* obsolete fn name */
...
int bcm_tlv_dot11_defrag(void *buf, uint buf_len, uint16 id,
	uint8 *out, uint *out_len, uint *data_len);

int bcm_tlv_dot11_frag_tot_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len);

int bcm_tlv_dot11_frag_tot_data_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len, uint *data_len);

int bcm_tlv_dot11_frag(uint16 id, const void *data, uint data_len,
	uint8 *buf, uint *buf_len);

int bcm_tlv_dot11_frag_subelement(uint16 subelement_id, const void *data, uint data_len,
	uint8 *buf, uint *buf_len);
int bcm_tlv_dot11_defrag_subelement(void *buf, uint buf_len, uint16 subelement_id,
		uint8 *out, uint *out_len, uint *data_len);
int bcm_tlv_dot11_frag_subelement_tot_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len);
int bcm_tlv_dot11_frag_subelement_tot_data_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len, uint *data_len);
