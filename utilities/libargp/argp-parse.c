...
static const struct argp_option argp_version_options[]
    = { { "version", 'V', 0, 0, N_("Print program version"), -1 }, { 0, 0, 0, 0, 0, 0 } };
    ...
    switch (key)
    {
        case 'V':
            if (argp_program_version_hook)
                ...
                __argp_error(
                    state,
                    dgettext(state->root_argp->argp_domain, "(PROGRAM ERROR) No version known!?"));
...
static const struct argp_option*
find_long_option(struct parser* parser, const char* arg, struct group** p)
{
    struct group* group;
...
static error_t
parser_init(
    struct parser* parser, const struct argp* argp, int argc, char** argv, int flags, void* input)
{
    error_t err = 0;
    struct group* group;
...
#define STORAGE(offset) ((void*) (((char*) parser->storage) + (offset)))
    ...
    for (group = parser->groups; group < parser->egroup && (!err || err == EBADKEY); group++)
    {
        if (group->parent)
...
static error_t
parser_finalize(struct parser* parser, error_t err, int arg_ebadkey, int* end_index)
{
    struct group* group;
    ...
    if (!err)
    {
        ...
        {
            for (group = parser->groups; group < parser->egroup && (!err || err == EBADKEY);
                 group++)
                if (group->args_processed == 0)
                    err = group_parse(group, &parser->state, ARGP_KEY_NO_ARGS, 0);
            for (group = parser->egroup - 1; group >= parser->groups && (!err || err == EBADKEY);
                 group--)
    ...
    if (err)
    {
        ...
        if (err == EBADKEY)
            ...
            __argp_state_help(&parser->state, parser->state.err_stream, ARGP_HELP_STD_ERR);
...
parser_parse_arg(struct parser* parser, char* val)
{
    ...
    for (group = parser->groups; group < parser->egroup && err == EBADKEY; group++)
    {
...
parser_parse_next(struct parser* parser, int* arg_ebadkey)
{
    ...
    /* Deal with short options. */
    {
        ...
        if (option->arg)
        {
            ...
            if (!value && !(option->flags & OPTION_ARG_OPTIONAL))
        ...
        return group_parse(group, &parser->state, option->key, value);
    }
    else
        ...
        if (parser->args_only)
        {
            *arg_ebadkey = 1;
            if (parser->state.next >= parser->state.argc)
                ...
                return parser_parse_arg(parser, parser->state.argv[parser->state.next]);
        }
        ...
        /* Look for options. */
        {
            ...
            switch (token)
            {
                ...
                case ARG_LONG_OPTION:
                {
                    ...
                    if (option->arg && !value && !(option->flags & OPTION_ARG_OPTIONAL))
                    /* We need an mandatory argument. */
                    {
                        if (parser->state.next == parser->state.argc)
                        /* Missing argument */
...
error_t
__argp_parse(
    const struct argp* argp, int argc, char** argv, unsigned flags, int* end_index, void* input)
{
    error_t err;
    struct parser parser;
...
weak_alias(__argp_input, _argp_input)
     ...
     * argp.h */
    void __THROW __argp_usage(__const struct argp_state* __state)
{
