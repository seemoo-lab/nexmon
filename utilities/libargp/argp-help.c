...
static struct uparams uparams
    = { DUP_ARGS,    DUP_ARGS_NOTE, SHORT_OPT_COL, LONG_OPT_COL, DOC_OPT_COL,
        ...
        while (*var)
        {
            ...
            if (isalpha(*var))
            {
                ...
                for (u = 0; u < nuparam_names; ++un, ++u)
                    if (strlen(un->name) == var_len && strncmp(var, un->name, var_len) == 0)
                    {
    ...
    if (opts)
    {
        ...
        for (o = opts, entry = hol->entries; !oend(o); entry++)
        {
            ...
            entry->group = cur_group
                = o->group ? o->group : ((!o->name && !o->key) ? cur_group + 1 : cur_group);
...
hol_add_cluster(struct hol* hol,
                int group,
                const char* header,
                int index,
                struct hol_cluster* parent,
...
static int
hol_cluster_is_child(const struct hol_cluster* cl1, const struct hol_cluster* cl2)
{
    while (cl1 && cl1 != cl2)
        cl1 = cl1->parent;
    return cl1 == cl2;
...
{
    if (hol->num_entries > 0)
        qsort(hol->entries, hol->num_entries, sizeof(struct hol_entry), hol_entry_qcmp);
}
    ...
    *cl_end = more->clusters;
    ...
    if (more->num_entries > 0)
    {
           ...
           non-shadowed SHORT_OPTIONS values.  */
        {
            ...
            __mempcpy(__mempcpy(entries, hol->entries, hol->num_entries * sizeof(struct hol_entry)),
                      more->entries,
...
{
    if (__argp_fmtstream_point(stream) + ensure >= __argp_fmtstream_rmargin(stream))
        __argp_fmtstream_putc(stream, '\n');
    else
        __argp_fmtstream_putc(stream, ' ');
}
...
/* If the option REAL has an argument, we print it in using the printf
   ...
   optional argument.  */
static void
arg(const struct argp_option* real,
    const char* req_fmt,
    const char* opt_fmt,
    const char* domain,
...
static const char*
filter_doc(const char* doc, int key, const struct argp* argp, const struct argp_state* state)
{
...
/* Prints STR as a header line, with the margin lines set appropriately, and
   ...
   to 0.  */
static void
print_header(const char* str, const struct argp* argp, struct pentry_state* pest)
{
...
comma(unsigned col, struct pentry_state* pest)
{
    if (pest->first)
    {
        ...
        if (cl && cl->header && *cl->header
            && (!pe || (pe->cluster != cl && !hol_cluster_is_child(pe->cluster, cl))))
...
static void
hol_entry_help(struct hol_entry* entry,
               const struct argp_state* state,
               argp_fmtstream_t stream,
               struct hol_help_state* hhstate)
{
        ...
        for (opt = real, num = entry->num; num > 0; opt++, num--)
            if (opt->name && ovisible(opt))
            {
                ...
                __argp_fmtstream_puts(
                    stream,
                    dgettext(state == NULL ? NULL : state->root_argp->argp_domain, opt->name));
            }
    }
    else
    /* A real long option.  */
    ...
    else
    {
        const char* tstr
            = real->doc
                  ? dgettext_safe(state == NULL ? NULL : state->root_argp->argp_domain, real->doc)
                  : 0;
...
static void
hol_help(struct hol* hol, const struct argp_state* state, argp_fmtstream_t stream)
{
    unsigned num;
    struct hol_entry* entry;
...
optional for any corresponding short options.");
        const char* fstr
            = filter_doc(tstr, ARGP_KEY_HELP_DUP_ARGS_NOTE, state ? state->root_argp : 0, state);
        if (fstr && *fstr)
        {
            __argp_fmtstream_putc(stream, '\n');
            __argp_fmtstream_puts(stream, fstr);
...
hol_usage(struct hol* hol, argp_fmtstream_t stream)
{
    if (hol->num_entries > 0)
    {
        ...
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0; entry++, nentries--)
            hol_entry_short_iterate(
                entry, add_argless_short_opt, entry->argp->argp_domain, &snao_end);
        ...
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0; entry++, nentries--)
            hol_entry_short_iterate(
                entry, usage_argful_short_opt, entry->argp->argp_domain, stream);

        /* Finally, a list of long options (whew!).  */
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0; entry++, nentries--)
            hol_entry_long_iterate(entry, usage_long_opt, entry->argp->argp_domain, stream);
    }
}
...
static int
argp_args_usage(const struct argp* argp,
                const struct argp_state* state,
                char** levels,
                int advance,
                argp_fmtstream_t stream)
...
static int
argp_doc(const struct argp* argp,
         const struct argp_state* state,
         int post,
         int pre_blank,
         int first_only,
         argp_fmtstream_t stream)
{
    ...
    /* We have to filter the doc strings.  */
    {
        ...
        text = (*argp->help_filter)(
            post ? ARGP_KEY_HELP_POST_DOC : ARGP_KEY_HELP_PRE_DOC, inp_text, input);
    }
    else
    ...
    if (child)
        while (child->argp && !(first_only && anything))
            anything |= argp_doc(
                (child++)->argp, state, post, anything || pre_blank, first_only, stream);
  ...
  __flockfile (stream);
    ...
    /* Print a short `Usage:' message.  */
    {
        ...
        do
        {
            ...
            if (first_pattern)
                __argp_fmtstream_printf(fs, "%s %s", dgettext(argp->argp_domain, "Usage:"), name);
            else
                __argp_fmtstream_printf(fs, "%s %s", dgettext(argp->argp_domain, "  or: "), name);
            ...
            {
                if (hol->num_entries > 0)
                    __argp_fmtstream_puts(fs, dgettext(argp->argp_domain, " [OPTION...]"));
            }
            else
...
Try `%s --help' or `%s --usage' for more information.\n"),
    ...
    if ((flags & ARGP_HELP_BUG_ADDR) && argp_program_bug_address)
    {
        ...
        __argp_fmtstream_printf(
            fs, dgettext(argp->argp_domain, "Report bugs to %s.\n"), argp_program_bug_address);
        anything = 1;
    }
...
void
__argp_help(const struct argp* argp, FILE* stream, unsigned flags, char* name)
{
...
void
__argp_failure(const struct argp_state* state, int status, int errnum, const char* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
