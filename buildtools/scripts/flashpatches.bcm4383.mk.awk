...
BEGIN {
	...
	fp_config_first_entry_ptr = strtonum(fp_config_first_entry_ptr);
	...
	fp_config_end_ptr_5 = strtonum(fp_config_end_ptr_5);
...
END {
    ...
    new_first_entry = fp_config_base + 8;
	printf "\t$(Q)printf %08x | xxd -r -p | dd of=$@ bs=1 conv=notrunc seek=$$((0x%08x - 0x%08x))\n", htonl(new_first_entry), fp_config_first_entry_ptr, ramstart;
	printf "\t$(Q)printf \"  PATCH fp_config_first_entry @ 0x%08x -> 0x%08x\\n\"\n", fp_config_first_entry_ptr, new_first_entry;
	...
	printf "\t$(Q)printf %08x | xxd -r -p | dd of=$@ bs=1 conv=notrunc seek=$$((0x%08x - 0x%08x))\n", htonl(fp_config_end), fp_config_end_ptr_5, ramstart;
	printf "\t$(Q)printf \"  PATCH fp_config_end_4 @ 0x%08x\\n\"\n", fp_config_end_ptr_5;
	printf "\n\nFORCE:\n"
}