...
class B43SymbolicScr:
	"""This class converts numeric SCR names into symbolic SCR names."""

	def __init__(self, header_file):
		"""The passed header_file parameter is a file path to the
		assembly file containing the symbolic SCR definitions."""
		try:
			defs = file(header_file).readlines()
		except IOError, e:
			print "B43SymbolicScr: Could not read %s: %s" % (e.filename, e.strerror)
			B43Exception
		# Parse the definitions
		self.scr_names = { }
		r = re.compile(r"#define\s+(\w+)\s+(r[0-9]+)")
		for line in defs:
			m = r.match(line)
			if not m:
				continue # unknown line
			name = m.group(1)
			offset = m.group(2)
			self.scr_names[offset.lower()] = name

	def get(self, scr):
		"""Get the symbolic name for an SCR. The scr parameter
		must be a string like "rXX", where XX is a number."""
		try:
			scr = self.scr_names[scr.lower()]
		except KeyError:
			pass # Symbol not found. Return numeric name.
		return scr
	...
	def __process_code(self):
		...
		for line in code:
			...
			for i in range(0, len(operands)):
				o = operands[i]
				# Transform SCR operands
				m = scr_re.match(o)
				if m:
					operands[i] = self.symScr.get(o)
					continue
