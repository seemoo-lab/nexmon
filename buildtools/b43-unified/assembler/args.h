...
enum movencoding {
	ENC_MOV_OR,	/* try using or when possible. */
	ENC_MOV_XOR,	/* use always xor. */
};
...
struct cmdline_args {
	...
	int rawivals;				/* Create wl initvals. */
	enum movencoding movenc;		/* Type of encoding for mov. */
	bool suppress_warnings;			/* Do not print warnings. */
};
