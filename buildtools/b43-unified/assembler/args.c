...
static void usage(void)
{
	...
	printf("  -m|--movenc ENC     Switch between using xor always (ENC=xor) or\n");
	printf("                      trying using or (ENC=or) when possible\n");
	printf("  -n|--nowarn         Suppress warnings\n");
...
int parse_args(int argc, char **argv)
{
	...
	for (i = 3; i < argc; i++) {
		if ((res = cmp_arg(argv, &i, "--help", "-h", NULL)) == ARG_MATCH) {
			...
			cmdargs.rawivals = 1;
		} else if ((res = cmp_arg(argv, &i, "--movenc", "-m", &param)) == ARG_MATCH) {
			if (strcasecmp(param, "xor") == 0)
				cmdargs.movenc = ENC_MOV_XOR;
			else if (strcasecmp(param, "or") == 0)
				cmdargs.movenc = ENC_MOV_OR;
			else {
				fprintf(stderr, "Invalid -m|--movenc\n\n");
				goto out_usage;
			}
