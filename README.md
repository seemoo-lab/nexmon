...
To be able to communicate with the firmwrae without root priviledges, we created a UDP interface accessible through the `libnexio` and used by `nexutil`. You first have to prove to the firmware that you generally have root priviledges by setting a securtiy cookie. Then you can use it for UDP based connections:
