...
extern int merge_ivs_files(JNIEnv* env, jobject thiz, char* file1, char* file2, char* output_file)
{
    int i, n, unused;
    unsigned long nbw;
    ...
    if( ( f_out = fopen( output_file, "wb+" ) ) == NULL )
    {
        perror( "fopen failed" );
        return( 1 );
    }
    ...
    {   
        ...
        if( ( f_in = fopen( act_file, "rb" ) ) == NULL )
        {
        	fclose(f_out);
            perror( "fopen failed" );
            return( 1 );
...
extern int create_ivs_from_pcap_file(JNIEnv* env, jobject thiz, char* pcap_file, char* output_file)
{
    time_t tt;
    int n, unused, ret;
    FILE *f_in;
    ...
    if( ( f_in = fopen( pcap_file, "rb" ) ) == NULL )
    {
        perror( "fopen failed" );
        return( 1 );
    }
    ...
    if( ( G.f_ivs = fopen( output_file, "wb+" ) ) == NULL )
    {
        perror( "fopen failed" );
        return( 1 );
    }
