...
int decrypt_pcap(char* _bssid, char* _essid, char* _passphrase, char* fileDir, int encryptionType)
{
    time_t tt;
    unsigned magic;
    char *s, buf[128];
    ...
    start_logger("myairdecap-ng logger");
   ...
   // opt.essid = _essid;
     memset(  opt.essid, 0, sizeof( opt.essid ) );
                strncpy( opt.essid, _essid, sizeof( opt.essid ) - 1 );

    opt.crypt = encryptionType;
    //opt.passphrase = _passphrase;
    memset(  opt.passphrase, 0, sizeof( opt.passphrase ) );
    strncpy( opt.passphrase, _passphrase, sizeof( opt.passphrase ) - 1 ); 
    ...
    if( n > 4 && ( n + 5 < (int) sizeof( buffer ) ) &&
        fileDir[n - 4] == '.' )
    {
        memcpy( buffer , fileDir, n - 4 );
        memcpy( buffer2, fileDir, n - 4 );
        memcpy( buffer  + n - 4, "-dec", 4 );
        memcpy( buffer2 + n - 4, "-bad", 4 );
        memcpy( buffer  + n, fileDir + n - 4, 5 );
        memcpy( buffer2 + n, fileDir + n - 4, 5 );
    }
    else
    {
        if( n > 5 && ( n + 6 < (int) sizeof( buffer ) ) &&
            fileDir[n - 5] == '.' )
        {
            memcpy( buffer , fileDir, n - 5 );
            memcpy( buffer2, fileDir, n - 5 );
            memcpy( buffer  + n - 5, "-dec", 4 );
            memcpy( buffer2 + n - 5, "-bad", 4 );
            memcpy( buffer  + n - 1, fileDir + n - 5, 6 );
            memcpy( buffer2 + n - 1, fileDir + n - 5, 6 );
        }
        else
        {
