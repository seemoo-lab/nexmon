...
struct apple80211req
{
        ...
        int32_t                 req_type;                               // 4 bytes
        int32_t                 req_val;                                // 4 bytes
        u_int64_t       req_len;                                // 8 bytes
        void       *req_data;                           // 8 bytes
};
...
int a80211_getset(uint32_t ioc, uint32_t type, uint32_t valuep, void *data, size_t length)
{
    ...
    strncpy(cmd.req_if_name, ifname, 16);
...
{
    return a80211_getset(SIOCSA80211, APPLE80211_IOC_CARD_SPECIFIC, cmd, buf, len);
}
