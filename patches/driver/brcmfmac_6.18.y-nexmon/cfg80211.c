...
#ifdef NEXMON_MON_IF
static s32
brcmf_cfg80211_nexmon_set_channel(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_chan_def *chandef) {
	struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(cfg_to_ndev(cfg));
	s32 err = 0;
	u16 chanspec;
...
static s32 brcmf_cfg80211_set_wiphy_params(struct wiphy *wiphy, int radio_idx, u32 changed)
{
	struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
