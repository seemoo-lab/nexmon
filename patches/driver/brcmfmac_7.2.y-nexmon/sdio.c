...
gotpkt:
	...
	if (bus->rxctl) {
		u8 *stale = bus->rxctl_orig;
		brcmf_dbg(CTL, "last control frame was unconsumed, replacing\n");
		bus->rxctl = buf + doff;
		bus->rxctl_orig = buf;
		bus->rxlen = len - doff;
...
brcmf_sdio_bus_txctl(struct device *dev, unsigned char *msg, uint msglen)
{
	...
	/* Clear any stale rxctl buffer from a previously cancelled or timed-out dcmd */
	spin_lock_bh(&bus->rxctl_lock);
	if (bus->rxctl) {
		u8 *stale = bus->rxctl_orig;
		bus->rxctl = NULL;
