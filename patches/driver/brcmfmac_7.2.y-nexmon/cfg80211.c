...
brcmf_cfg80211_nexmon_set_channel(struct wiphy *wiphy, struct net_device *, struct cfg80211_chan_def *chandef) {
	...
	/* A hop can re-gate the D11 clocks (mpc/PM). Keep them off so
	 * airodump RX stays live and injected frames still key. */
	brcmf_fil_cmd_int_set(ifp, BRCMF_C_SET_PM, 0);
