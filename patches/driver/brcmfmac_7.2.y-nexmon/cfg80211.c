...
/* Kernel 7.1 converted a group of cfg80211_ops from taking the interface's
 * struct net_device * to taking its struct wireless_dev *, and moved
 * cfg80211_new_sta()/cfg80211_del_sta() over with them. Every op body below
 * wants the netdev, so take the parameter through these macros and resolve
 * the netdev once at the top of each function - one body then compiles on
 * both sides of the change. BRCMF_OP_NDEV_PASS() is the reverse direction:
 * it turns a netdev back into whatever these callees now expect.
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7,1,0)
#define BRCMF_OP_NDEV_ARG		struct wireless_dev *wdev
#define BRCMF_OP_NDEV			(wdev->netdev)
#define BRCMF_OP_NDEV_PASS(ndev)	((ndev)->ieee80211_ptr)
#else
#define BRCMF_OP_NDEV_ARG		struct net_device *__ndev
#define BRCMF_OP_NDEV			(__ndev)
#define BRCMF_OP_NDEV_PASS(ndev)	(ndev)
#endif
...
brcmf_cfg80211_nexmon_set_channel(struct wiphy *wiphy, struct net_device *, struct cfg80211_chan_def *chandef) {
	...
	/* PM and mpc are already forced off once in brcmf_net_mon_open() when
	 * monitor mode is enabled, and nothing on the hop path turns them back
	 * on. Re-asserting both here on *every* hop tripled the DCMD count of
	 * this op (three firmware round-trips instead of one), and under a fast
	 * multi-band hopper like airodump-ng that DCMD storm - colliding with
	 * background get_channel polling on the single-outstanding BCDC/SDIO
	 * path - is what drives the firmware into -ETIMEDOUT (-110) and a proto
	 * desync that only a driver reload clears. Rely on the one-time setup in
	 * brcmf_net_mon_open() instead; if a future firmware is found to re-gate
	 * the D11 clocks on a chanspec change, re-assert periodically (every Nth
	 * hop) rather than every hop. */
...
void brcmf_set_mpc(struct brcmf_if *ifp, int mpc)
{
	...
	/* Scan completion / iface down re-enable mpc. That gates the D11
	 * clocks so injected frames are echoed to pcap but never keyed. */
	if (mpc) {
		for (i = 0; i < BRCMF_MAX_IFS; i++) {
			struct brcmf_if *tmp = drvr->iflist[i];

			if (tmp && tmp->vif &&
			    tmp->vif->wdev.iftype == NL80211_IFTYPE_MONITOR)
				mpc = 0;
		}
	}
...
static s32
brcmf_cfg80211_del_key(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
		       int link_id, u8 key_idx, bool pairwise, const u8 *mac_addr)
{
	struct net_device *ndev = BRCMF_OP_NDEV;
	struct brcmf_if *ifp = netdev_priv(ndev);
	struct brcmf_wsec_key *key;
	s32 err;
...
static s32
brcmf_cfg80211_add_key(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
		       int link_id, u8 key_idx, bool pairwise, const u8 *mac_addr,
		       struct key_params *params)
{
	...
	if (params->key_len == 0)
		return brcmf_cfg80211_del_key(wiphy, BRCMF_OP_NDEV_PASS(ndev), -1,
					      key_idx, pairwise, mac_addr);
...
static s32
brcmf_cfg80211_get_key(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
...
static s32
brcmf_cfg80211_get_station(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
...
static int
brcmf_cfg80211_dump_station(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
			    int idx, u8 *mac, struct station_info *sinfo)
{
	...
	if (idx < le32_to_cpu(cfg->assoclist.count)) {
		...
		return brcmf_cfg80211_get_station(wiphy, BRCMF_OP_NDEV_PASS(ndev),
						  mac, sinfo);
	}
	return -ENOENT;
}
...
static int
brcmf_cfg80211_del_station(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
...
static int
brcmf_cfg80211_change_station(struct wiphy *wiphy, BRCMF_OP_NDEV_ARG,
...
static int
/* Kernel 7.3 changed the mgmt_tx op to pass the cookie by value (generated by
 * cfg80211) instead of an output pointer the driver fills in. Normalise it to a
 * local tx_cookie so the body below is version-independent.
 */
	...
	*cookie = 0;
	...
	if (ieee80211_is_probe_resp(mgmt->frame_control)) {
		...
		err = brcmf_vif_set_mgmt_ie(vif,
					    ...
					    &buf[ie_offset],
					    ie_len);
		cfg80211_mgmt_tx_status(wdev, tx_cookie, buf, len, true,
					GFP_KERNEL);
		...
		/* Add the packet Id */
		action_frame->packet_id = cpu_to_le32(tx_cookie);
		/* Add BSSID */
		...
		brcmf_dbg(TRACE, "Action frame, cookie=%lld, len=%d, freq=%d\n",
			  tx_cookie, le16_to_cpu(action_frame->len), freq);

		ack = brcmf_p2p_send_action_frame(cfg, cfg_to_ndev(cfg),
						  af_params);

		cfg80211_mgmt_tx_status(wdev, tx_cookie, buf, len, ack,
					GFP_KERNEL);
		kfree(af_params);
	} else {
			       ...
			       const struct brcmf_event_msg *e, void *data)
{
	...
	if (((event == BRCMF_E_ASSOC_IND) || (event == BRCMF_E_REASSOC_IND)) &&
		...
		cfg80211_new_sta(BRCMF_OP_NDEV_PASS(ndev), e->addr, sinfo,
				 GFP_KERNEL);
		...
		cfg80211_del_sta(BRCMF_OP_NDEV_PASS(ndev), e->addr, GFP_KERNEL);
	}
	return 0;
}
