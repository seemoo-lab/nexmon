...
wlc_doioctl_hook(void *wlc, int cmd, char *arg, int len, void *wlc_if)
{
    ...
    switch (cmd) {
        ...
        case 900: /* copy memory from offset into argument buffer */
        {
            uint32 offset;
            if (len > 0) {
                offset = *(uint32 *)arg;
                memcpy(arg, (void *)offset, len);
                ret = IOCTL_SUCCESS;
            }
            break;
        }
...
__attribute__((at(0x457A98, "", CHIP_VER_BCM4390b1, FW_VER_27_10_1121_57_26_g9f20a98)))
